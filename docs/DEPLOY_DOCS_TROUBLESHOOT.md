...
`./deploy-docs.sh --auto-detect` 실행 시 다음 메시지가 나타나고 종료됩니다:
...
| 상황 | 해결책 |
|------|--------|
| 파일 수정 후 바로 | `git add . && ./deploy-docs.sh --auto-detect` |
| 커밋 후 | `./deploy-docs.sh --test-filter 'TestName'` |
