...
public class BatchCrawlerService {
    ...
    private String batchCrawlerBaseUrl;
    
    /**
     * 배치 서버로 파일을 업로드하고 배치 작업 시작
     * 
     * @param file 업로드할 JSON 파일
     * @return 배치 작업 응답
     */
    public BatchJobResponse uploadAndStartBatch(MultipartFile file) throws IOException {
        String url = batchCrawlerBaseUrl + "/api/v1/batch/crawler/upload-and-import";
        
        log.info("Uploading file to batch server: {} (size: {} bytes)", 
        ...
        try {
            ...
            if (response.getStatusCode().is2xxSuccessful()) {
                ...
                log.info("Batch job started successfully: {}", responseBody);
                ...
                return BatchJobResponse.builder()
                        .success(true)
                        .message(responseBody != null ? responseBody : "배치 작업이 시작되었습니다.")
                        .jobId(jobId)
                        .build();
            } else {
            ...
            log.error("Failed to communicate with batch server", e);
