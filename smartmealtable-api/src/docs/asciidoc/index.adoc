...
include::{snippets}/onboarding-food-preferences-post/http-request.adoc[]

===== 요청 필드

include::{snippets}/onboarding-food-preferences-post/request-fields.adoc[]

==== 성공 응답 (201 Created)

include::{snippets}/onboarding-food-preferences-post/http-response.adoc[]

===== 응답 필드

include::{snippets}/onboarding-food-preferences-post/response-fields.adoc[]
...
include::{snippets}/budget/update-daily-with-date-success/http-request.adoc[]

===== 경로 파라미터

include::{snippets}/budget/update-daily-with-date-success/path-parameters.adoc[]

===== 요청 헤더

include::{snippets}/budget/update-daily-with-date-success/request-headers.adoc[]

===== 요청 필드

include::{snippets}/budget/update-daily-with-date-success/request-fields.adoc[]

==== 성공 응답 (200 OK)

include::{snippets}/budget/update-daily-with-date-success/http-response.adoc[]

===== 응답 필드

include::{snippets}/budget/update-daily-with-date-success/response-fields.adoc[]

==== 에러 응답

===== 날짜 형식 오류 (400 Bad Request)

include::{snippets}/budget/update-daily-with-date-invalid-date/http-response.adoc[]

include::{snippets}/budget/update-daily-with-date-invalid-date/response-fields.adoc[]

===== 예산 검증 실패 (422 Unprocessable Entity)

include::{snippets}/budget/update-daily-with-date-validation-error/http-response.adoc[]

include::{snippets}/budget/update-daily-with-date-validation-error/response-fields.adoc[]

==== 참고

* `applyForward: true`를 선택하면, 해당 날짜부터 미래의 모든 날짜(연말까지)에 동일한 예산이 일괄 적용됩니다.
* 이미 개별 설정된 날짜가 있더라도 덮어씌워집니다.
* 응답의 `affectedDatesCount` 필드는 일괄 적용된 날짜의 개수를 나타냅니다.
...
장바구니에서 직접 지출 내역을 등록합니다. 가게 ID와 음식 ID를 포함하여 저장되므로 상세 페이지 링크가 가능합니다.

==== 요청

include::{snippets}/expenditure/create-from-cart-success/http-request.adoc[]

===== 요청 헤더

include::{snippets}/expenditure/create-from-cart-success/request-headers.adoc[]

===== 요청 필드

include::{snippets}/expenditure/create-from-cart-success/request-fields.adoc[]

==== 성공 응답 (201 Created)

include::{snippets}/expenditure/create-from-cart-success/http-response.adoc[]

===== 응답 필드

include::{snippets}/expenditure/create-from-cart-success/response-fields.adoc[]

==== 에러 응답

===== 필수 필드 누락 (400 Bad Request)

include::{snippets}/expenditure/create-from-cart-missing-field/http-response.adoc[]

include::{snippets}/expenditure/create-from-cart-missing-field/response-fields.adoc[]

===== 유효성 검증 실패 (422 Unprocessable Entity)

include::{snippets}/expenditure/create-from-cart-validation-failed/http-response.adoc[]

include::{snippets}/expenditure/create-from-cart-validation-failed/response-fields.adoc[]

==== 참고

* `hasStoreLink`: `true`일 경우 가게 상세 페이지로 이동 가능
* `hasFoodLink`: 각 항목별로 음식 상세 페이지 링크 가능 여부 표시
* 일반 지출 내역과 달리 가게 및 음식 정보가 연계되어 있어 추후 추천에 활용 가능
