...
class RecommendationControllerRestDocsTest extends AbstractRestDocsTest {
    ...
    private static FieldDescriptor[] getRecommendationListResponseFields() {
        return new FieldDescriptor[]{
                ...
                fieldWithPath("data[].categoryName").type(JsonFieldType.STRING).description("카테고리 이름").optional(),
                fieldWithPath("data[].address").type(JsonFieldType.STRING).description("주소").optional(),
                fieldWithPath("data[].recommendationScore").type(JsonFieldType.NUMBER).description("최종 추천 점수 (0-100)"),
                fieldWithPath("data[].scores").type(JsonFieldType.OBJECT).description("추천 점수 상세").optional(),
                fieldWithPath("data[].scores.stability").type(JsonFieldType.NUMBER).description("안정성 점수 (0-100)").optional(),
                fieldWithPath("data[].scores.exploration").type(JsonFieldType.NUMBER).description("탐험성 점수 (0-100)").optional(),
                fieldWithPath("data[].scores.budgetEfficiency").type(JsonFieldType.NUMBER).description("예산 효율성 점수 (0-100)").optional(),
                fieldWithPath("data[].scores.accessibility").type(JsonFieldType.NUMBER).description("접근성 점수 (0-100)").optional(),
                ...
                fieldWithPath("data[].isFavorite").type(JsonFieldType.BOOLEAN).description("즐겨찾기 여부"),
                fieldWithPath("data[].isOpen").type(JsonFieldType.BOOLEAN).description("현재 영업 여부"),
    ...
    void setUpTestData() {
        ...
        // 회원 생성
        Member testMember = Member.create(savedGroup.getGroupId(), "추천테스트유저", null, RecommendationType.BALANCED);
        ...
        store2 = Store.builder()
                .name("신선한 일식당")
                .categoryIds(java.util.List.of(japaneseCategory.getCategoryId()))
