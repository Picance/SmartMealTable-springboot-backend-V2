...
class CartControllerTest extends AbstractRestDocsTest {
    ...
    @Test
    @DisplayName("장바구니 아이템 추가 - replaceCart 사용 (다른 가게 상품 교체 - 성공)")
    void addCartItem_replaceCart_success() throws Exception {
        // given: 첫 번째 가게에 상품 추가
        AddCartItemRequest firstRequest = AddCartItemRequest.builder()
                .storeId(storeId)
                .foodId(foodId1)
                .quantity(2)
                .build();
        ...
        Store store2 = Store.builder()
                .name("맛있는 중식당")
                .categoryIds(java.util.List.of(category.getCategoryId()))
                .sellerId(2L)
        ...
        // when & then: 두 번째 가게 상품을 replaceCart=true로 추가
        AddCartItemRequest replaceRequest = AddCartItemRequest.builder()
                .storeId(savedStore2.getStoreId())
                .foodId(savedFood3.getFoodId())
                .quantity(1)
                .replaceCart(true)
                .build();
        ...
        mockMvc.perform(post("/api/v1/cart/items")
                ...
                .andExpect(jsonPath("$.result").value("SUCCESS"))
                .andExpect(jsonPath("$.data.cartId").exists())
                .andExpect(jsonPath("$.data.foodId").value(savedFood3.getFoodId()))
                .andExpect(jsonPath("$.data.quantity").value(1))
                .andExpect(jsonPath("$.data.replacedCart").value(true))
                .andDo(document("cart-add-item-replace-success",
                        getDocumentRequest(),
                        getDocumentResponse(),
                        authorizationHeader(),
                        requestFields(
                                ...
                                fieldWithPath("foodId").description("음식 ID"),
                                fieldWithPath("quantity").description("수량"),
                                fieldWithPath("replaceCart").description("기존 장바구니 교체 여부 (기본값: false)")
                        ),
                        responseFields(
                                fieldWithPath("result").description("응답 결과 (SUCCESS/FAIL)"),
                                fieldWithPath("data.cartId").description("장바구니 ID"),
    ...
    @Test
    @DisplayName("장바구니 아이템 추가 - replaceCart 미사용 (다른 가게 상품 충돌 - 실패)")
    void addCartItem_cartConflict_failure() throws Exception {
        // given: 첫 번째 가게에 상품 추가
        AddCartItemRequest firstRequest = AddCartItemRequest.builder()
                .storeId(storeId)
                .foodId(foodId1)
        ...
        // when & then: 두 번째 가게 상품을 replaceCart=false로 추가 (기본값)
        AddCartItemRequest conflictRequest = AddCartItemRequest.builder()
                .storeId(savedStore2.getStoreId())
                .foodId(savedFood3.getFoodId())
                .quantity(1)
                .replaceCart(false)
        ...
        mockMvc.perform(post("/api/v1/cart/items")
                ...
                .andExpect(jsonPath("$.error.message").value("장바구니에 다른 가게의 상품이 있습니다. replaceCart=true로 설정하면 기존 장바구니를 비우고 추가할 수 있습니다."));
    }

    @Test
    @DisplayName("장바구니 아이템 추가 - 같은 가게 상품은 replaceCart 무관")
    void addCartItem_sameSstore_replaceCartIgnored() throws Exception {
        // given: 첫 번째 상품 추가
        AddCartItemRequest firstRequest = AddCartItemRequest.builder()
                .storeId(storeId)
                .foodId(foodId1)
        ...
        // when & then: 같은 가게의 다른 상품 추가 (replaceCart=true여도 적용 안 됨)
        AddCartItemRequest sameStoreRequest = AddCartItemRequest.builder()
                .storeId(storeId)
                .foodId(foodId2)
                .quantity(1)
                .replaceCart(true)
        ...
        mockMvc.perform(post("/api/v1/cart/items")
                        .header("Authorization", accessToken)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(sameStoreRequest)))
                .andDo(print())
                .andExpect(status().isCreated())
                .andExpect(jsonPath("$.result").value("SUCCESS"))
                .andExpect(jsonPath("$.data.foodId").value(foodId2))
                .andExpect(jsonPath("$.data.quantity").value(1))
                .andExpect(jsonPath("$.data.replacedCart").value(false));

        // 검증: 두 상품 모두 장바구니에 있어야 함
        mockMvc.perform(get("/api/v1/cart")
    ...
    @Test
    @DisplayName("장바구니 아이템 추가 - replaceCart 기본값은 false")
    void addCartItem_replaceCart_defaultFalse() throws Exception {
        // given: 첫 번째 가게에 상품 추가
        AddCartItemRequest firstRequest = AddCartItemRequest.builder()
                .storeId(storeId)
                .foodId(foodId1)
        ...
        // when & then: replaceCart 필드 생략하고 추가 (기본값 false)
        AddCartItemRequest defaultRequest = AddCartItemRequest.builder()
                .storeId(savedStore2.getStoreId())
                .foodId(savedFood3.getFoodId())
                .quantity(1)
                .build();
