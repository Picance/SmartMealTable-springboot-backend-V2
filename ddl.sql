...
CREATE TABLE store (
    ...
    store_name            VARCHAR(100)   NOT NULL COMMENT '음식점의 상호명',
    main_image_url        VARCHAR(255)   NULL     COMMENT '음식점의 대표 이미지 주소',
    review_count          INT            NOT NULL DEFAULT 0 COMMENT '음식점에 달린 리뷰의 총 개수',
    view_count            BIGINT         NOT NULL DEFAULT 0 COMMENT '총 조회수 (추천 알고리즘용)',
    average_price         INT            NULL     COMMENT '음식점의 1인당 평균 가격대',
    lot_number_address    VARCHAR(255)   NULL     COMMENT '지번 기준 주소',
    street_name_address   VARCHAR(255)   NOT NULL COMMENT '도로명 기준 주소',
    detailed_address      VARCHAR(255)   NULL     COMMENT '건물 호수 등 상세 주소',
    latitude              DECIMAL(9,6)   NOT NULL COMMENT '주소의 위도 정보',
    longitude             DECIMAL(9,6)   NOT NULL COMMENT '주소의 경도 정보',
    phone_number          VARCHAR(20)    NULL     COMMENT '음식점의 연락처',
    registered_dt         DATETIME       NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '가게 등록일 (시스템 자동 기록, 추천 알고리즘의 신규성 점수 계산용, 비즈니스 필드)',
    created_at            DATETIME       NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '감사 필드 (도메인에 노출 안 함)',
    updated_at            DATETIME       NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '감사 필드 (도메인에 노출 안 함)',
...
CREATE TABLE store_opening_hour (
                                    ...
                                    day_of_week           VARCHAR(20) NOT NULL COMMENT 'MONDAY, TUESDAY, ..., SUNDAY, EVERYDAY',
                                    start_time            TIME        NOT NULL COMMENT '예: ''11:00:00''',
                                    end_time              TIME        NOT NULL COMMENT '예: ''21:00:00''',
                                    break_start_time      TIME        NULL     COMMENT '브레이크 타임이 없는 경우 NULL',
                                    break_end_time        TIME        NULL     COMMENT '브레이크 타임이 없는 경우 NULL',
                                    last_order_time       TIME        NULL     COMMENT '마지막 주문 시간이 없는 경우 NULL',
                                    created_at            DATETIME    NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '감사 필드 (도메인에 노출 안 함)',
                                    updated_at            DATETIME    NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '감사 필드 (도메인에 노출 안 함)',
...
CREATE TABLE food (
                      ...
                      description   VARCHAR(255)   NULL     COMMENT '음식에 대한 상세 설명',
                      image_url     VARCHAR(255)   NULL     COMMENT '음식 이미지 주소',
...
CREATE TABLE store_temporary_closure (
                                         ...
                                         closure_start_date         DATE     NOT NULL COMMENT '휴무 시작 날짜',
                                         closure_end_date           DATE     NOT NULL COMMENT '휴무 종료 날짜',
                                         reason                     VARCHAR(255) NULL COMMENT '휴무 사유',
                                         ...
                                         INDEX idx_store_id (store_id),
                                         INDEX idx_closure_dates (store_id, closure_start_date, closure_end_date),
                                         INDEX idx_registered_at (registered_at)
